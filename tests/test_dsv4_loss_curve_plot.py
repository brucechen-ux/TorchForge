...
def test_loss_curve_svg_contains_both_projects_and_distinct_markers(tmp_path: Path) -> None:
    ...
    assert "B: TorchForge vs peer" in svg
...
def test_multi_series_svg_facets_overlapping_curves(tmp_path: Path) -> None:
    ...
    for label in ("A", "B", "C"):
        assert f"{label}: TorchForge vs peer" in svg
