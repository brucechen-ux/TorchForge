...
class MQA(nn.Module):
    """Multi-query attention as a directly instantiable PyTorch module.
    ...
    def __init__(
        ...
        *,
        ...
        self.v_proj = nn.Linear(hidden_size, self.value_head_dim, bias=bias)
    ...
    def _shape_k(self, key: torch.Tensor) -> torch.Tensor:
        ...
        return key.view(batch_size, seq_length, 1, self.head_dim).transpose(1, 2)

    def _shape_v(self, value: torch.Tensor) -> torch.Tensor:
        batch_size, seq_length = value.shape[:-1]
        return value.view(batch_size, seq_length, 1, self.value_head_dim).transpose(1, 2)
    ...
    def _apply_rotary(
        ...
        if position_embeddings is None:
            raise ValueError("MQA requires position_embeddings when rotary=True.")
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        key = _repeat_kv(key, self.num_attention_heads)
        value = _repeat_kv(value, self.num_attention_heads)
